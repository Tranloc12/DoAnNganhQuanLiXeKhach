...
public class PaypalController {
    ...
    /**
     * Endpoint để tạo một thanh toán PayPal. Phương thức này nhận yêu cầu POST
     * từ front-end để khởi tạo giao dịch PayPal.
     *
     * @param price Giá tiền của giao dịch.
     * @param bookingId ID của Booking liên quan đến thanh toán này.
     * @return ResponseEntity chứa URL phê duyệt của PayPal nếu thành công, hoặc
    ...
    /**
     * Endpoint để thực hiện thanh toán sau khi người dùng phê duyệt trên
     * PayPal. Phương thức này nhận yêu cầu GET từ front-end sau khi PayPal
     * chuyển hướng người dùng về.
     *
     * @param paymentId ID thanh toán từ PayPal, được truyền qua URL parameter.
     * @param payerId ID người trả tiền từ PayPal, được truyền qua URL
     * parameter.
     * @param bookingId ID của Booking liên quan, được truyền qua URL parameter.
     * @return ResponseEntity chứa thông báo thành công hoặc lỗi sau khi thực
     * hiện thanh toán.
     */
    ...
    public void successPay(@RequestParam("paymentId") String paymentId,
        ...
        try {
            com.paypal.api.payments.Payment executedPayment
                    = paypalService.executePaymentAndSaveToDb(paymentId, payerId, bookingId);

            if ("approved".equals(executedPayment.getState())) {
                // Redirect về frontend báo thành công
                response.sendRedirect("http://localhost:3000/success?bookingId=" + bookingId);
            } else {
                response.sendRedirect("http://localhost:3000/fail?bookingId=" + bookingId);
            }
        } catch (Exception e) {
            response.sendRedirect("http://localhost:3000/error?bookingId=" + bookingId);
        }
    }

    @GetMapping("/cancel")
    public void cancelPay(@RequestParam("bookingId") Integer bookingId,
            HttpServletResponse response) throws IOException {
        response.sendRedirect("http://localhost:3000/cancel?bookingId=" + bookingId);
    }
}
