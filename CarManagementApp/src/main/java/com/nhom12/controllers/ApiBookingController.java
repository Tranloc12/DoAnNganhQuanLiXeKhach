...
public class ApiBookingController {
    ...
    public ResponseEntity<?> createBooking(@RequestBody BookingRequest bookingRequest, Principal principal) {
        ...
        try {
            ...
            if (booking == null) {
                return ResponseEntity.status(500).body("Đặt chỗ thất bại do BookingService trả về null (có thể hết ghế hoặc lỗi DB).");
            }
            ...
            responseMap.put("id", booking.getId());
            responseMap.put("bookingId", booking.getId());
            responseMap.put("message", "Đặt vé thành công");
            return ResponseEntity.ok(responseMap);

        } catch (Exception e) {
            e.printStackTrace();
            String errorMsg = "Lỗi Backend Exception: " + e.getMessage();
            if (e.getCause() != null) {
